# Runtime Tcl commands to interact with - Encryption_Core_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/Encryption_Core_v1_0_include.tcl
...
# Test: data_in
# Create a write transaction at data_in_addr address
create_hw_axi_txn w_data_in_addr [get_hw_axis $jtag_axi_master] -type write -address $data_in_addr -data $wdata_1
# Create a read transaction at data_in_addr address
create_hw_axi_txn r_data_in_addr [get_hw_axis $jtag_axi_master] -type read -address $data_in_addr
# Initiate transactions
run_hw_axi r_data_in_addr
run_hw_axi w_data_in_addr
run_hw_axi r_data_in_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_data_in_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - data_in"
} else {
	puts "Data comparison test fail for - data_in, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}

# Test: key_in
# Create a write transaction at key_in_addr address
create_hw_axi_txn w_key_in_addr [get_hw_axis $jtag_axi_master] -type write -address $key_in_addr -data $wdata_1
# Create a read transaction at key_in_addr address
create_hw_axi_txn r_key_in_addr [get_hw_axis $jtag_axi_master] -type read -address $key_in_addr
# Initiate transactions
run_hw_axi r_key_in_addr
run_hw_axi w_key_in_addr
run_hw_axi r_key_in_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_key_in_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - key_in"
} else {
	puts "Data comparison test fail for - key_in, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: data_out
set wdata_data_out [get_init_data 0]
create_hw_axi_txn w_data_out_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_data_out}
create_hw_axi_txn r_data_out_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_data_out_addr
run_hw_axi w_data_out_addr
run_hw_axi r_data_out_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_data_out_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - data_out"
} else {
	puts "Data comparison test fail for - data_out, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
